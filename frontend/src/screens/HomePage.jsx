      ...
      .then((res) => {
        if (res.data.status === true) {
          ...
          for (let i = 0; i < _data.length; i++) {
            const values = {
              label: _data[i].firstName + " " + _data[i].lastName,
              value: _data[i]._id,
            };
            data.push(values);
          }
  ...
  return (
    <div className="HomePage">
                ...
                <Card.Body>
                  ...
                  {user.role == "ADMIN" ? (
                    <div className="d-flex justify-content-between align-items-center mt-3">
                      <Button className="col-md-8 col-sm-6 primary-btn bg-primary-1 text-white border">
                        Open
                      </Button>
                      <Button
                        className="primary-btn bg-primary-1 text-white border"
                        onClick={() => editData(key)}
                      >
                        <FaEdit />
                      </Button>
                      <Button className="primary-btn bg-primary-1 text-white border">
                        <FaTrashAlt />
                      </Button>
                    </div>
                  ) : (
                    <div className="d-flex justify-content-between align-items-center mt-3">
                      <Button className="col-md-12 col-sm-12 primary-btn bg-primary-1 text-white border">
                        Open
                      </Button>
                    </div>
                  )}
      ...
      <Modal show={show} onHide={!show}>
        ...
        <Modal.Footer>
          <Button
            className="primary-btn bg-white text-primary-1 border-1 border-color-primary"
